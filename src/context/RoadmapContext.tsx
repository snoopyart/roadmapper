...
export function RoadmapProvider({ children }: { children: ReactNode }) {
  ...
  const [savedRoadmaps, setSavedRoadmaps] = useReducer(
    (state: RoadmapConfig[], action: { type: string; payload?: RoadmapConfig | string }) => {
      switch (action.type) {
        case 'UPDATE':
        ...
        case 'SET':
          return action.payload as unknown as RoadmapConfig[];
        default:
          return state;
      }
    },
  ...
  // Save current roadmap to the list whenever it changes
  useEffect(() => {
  ...
  }, [savedRoadmaps, currentRoadmapId]);
  ...
  const createNewRoadmap = useCallback(() => {
  ...
  const duplicateRoadmap = useCallback(() => {
  ...
  const deleteRoadmap = useCallback((id: string) => {
    if (savedRoadmaps.length <= 1) {
      // Can't delete the last roadmap, reset instead
      const newRoadmap = createDefaultRoadmap();
      setSavedRoadmaps({ type: 'SET', payload: [newRoadmap] as unknown as RoadmapConfig });
      setCurrentRoadmapId(newRoadmap.id);
      dispatch({ type: 'LOAD_STATE', payload: newRoadmap });
      return;
    }
  ...
  const importFromShare = useCallback((data: Partial<RoadmapConfig>) => {
  ...
  }, [sharedDataChecked, importFromShare]);
